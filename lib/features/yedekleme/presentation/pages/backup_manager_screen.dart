...
class _BackupManagerScreenState extends State<BackupManagerScreen> {
  ...
  Future<void> _deleteBackup(String fileName) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        title:
            Text(loc.translate('menu_deleteBackupConfirmTitle') ?? 'Yedek Sil'),
        content: Text(
  ...
  Future<void> _importExternalBackup() async {
    ...
    try {
      ...
      final newPath =
          await DatabaseBackupRestore.instance.copyToBackupFolder(file);
      final newFileName = newPath.split('/').last;

      // ✅ restoreDatabase fonksiyonu artık backup klasöründen çalışacak
      final success =
          await DatabaseBackupRestore.instance.restoreDatabase(newFileName);
                        ...
                        trailing: Row(
                          ...
                          children: [
                            IconButton(
                              ...
                              onPressed: () async {
                                await DatabaseBackupRestore.shareBackupFile(
                                    name);
                              },
                            ),
                            IconButton(
