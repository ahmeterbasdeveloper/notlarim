...
class KategoriRepositoryImpl extends BaseRepositoryImpl<Kategori>
  ...
  @override
  Future<List<Kategori>> searchKategoriler(String query) async {
    final database = await db;
    final result = await database.query(
      tableName,
      // Hem başlıkta hem açıklamada arama yapar
      where:
          '${KategoriAlanlar.baslik} LIKE ? OR ${KategoriAlanlar.aciklama} LIKE ?',
